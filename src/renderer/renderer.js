...
function bindIPC() {
  window.projectApi.onTriggerCapture(() => {
    console.log('[snitch][renderer] received trigger-capture');
    state.pendingTrigger = true;
    startCapture();
  });
  window.projectApi.onTriggerCaptureMenu((options = {}) => {
    console.log('[snitch][renderer] received trigger-capture-menu');
    showWindow();
