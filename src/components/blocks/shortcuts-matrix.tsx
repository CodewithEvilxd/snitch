  ...
  return (
    <section className="relative z-10 w-full py-20 px-6 border-t border-neutral-200/70 dark:border-neutral-800/70 bg-neutral-50/50 dark:bg-neutral-950/40">
      <div className="max-w-[1120px] mx-auto flex flex-col items-center text-center">
        ...
        <span className="font-ndot text-xs tracking-widest text-neutral-400 dark:text-neutral-500 uppercase mb-3">
          [ POWER-USER SHORTCUTS ]
        </span>
        <h2 className="font-ndot text-3xl sm:text-4xl uppercase tracking-wide text-neutral-900 dark:text-white max-w-xl">
          Everything at your fingertips.
        </h2>
        <p className="font-ntype text-sm text-neutral-500 dark:text-neutral-400 max-w-md mt-2 mb-12">
          ...
          {shortcuts.map((s) => (
            ...
            >
              <div className="flex items-center justify-between w-full mb-2.5">
                <kbd className="inline-flex items-center justify-center min-w-[32px] h-[28px] px-2 rounded-md bg-neutral-100 dark:bg-neutral-800 border border-neutral-300/80 dark:border-neutral-700 font-ndot text-xs text-neutral-800 dark:text-neutral-200 shadow-sm">
                  {s.key}
                </kbd>
                <span className="h-1.5 w-1.5 rounded-full bg-neutral-300 dark:bg-neutral-700" />
              </div>
              <span className="font-ndot text-xs uppercase tracking-wider text-neutral-900 dark:text-neutral-100">
                {s.label}
              </span>
              <span className="font-ntype text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                {s.desc}
              </span>
            </div>
          ))}
